...
| 카카오 로그인 | — | 구글 옆 카카오 로그인 (code→커스텀 토큰) | `lib/kakao.ts`, `app/kakao/page.tsx`, `tg-post-saver/kakao-auth.js` | Firebase 미지원 → 백엔드가 Firebase 커스텀 토큰 발급, uid=`kakao_{id}`. 시크릿은 Render env 에만 |
